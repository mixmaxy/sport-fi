...
import { useCreateActivity, useUpdateActivity } from '@/features/activity/services/activityApi';
...
export const ActivityFormModal = ({ activity, onClose }: Props) => {
    ...
    const [imageUrls, setImageUrls] = useState<string[]>(activity?.imageUrls || []);
    ...
    const { register, handleSubmit, watch, formState: { errors } } = useForm<ActivityFormData>({
    ...
    const handleAddImage = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        } catch { setUploadError('Gagal mengunggah gambar.'); }
    ...
    const onSubmit = (data: ActivityFormData) => {
        if (imageUrls.length === 0) { setUploadError('Tambahkan minimal 1 gambar.'); return; }
        setServerError(null);
            ...
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] flex flex-col">
                ...
                <div className="overflow-y-auto p-6 space-y-5 flex-1">
                    ...
                    <div>
                        ...
                        <input ref={imgInputRef} type="file" accept="image/*" onChange={handleAddImage} className="hidden" />
                        {uploadError && <p className="text-sm text-red-600">{uploadError}</p>}
                    </div>

                    <Input {...register('title')} label="Judul Aktivitas" error={errors.title?.message} required />
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <Input {...register('price')} type="number" label="Harga (Rp)" error={errors.price?.message} required />
                        <Input {...register('price_discount')} type="number" label="Harga Diskon (Rp)" error={errors.price_discount?.message} helperText="Isi 0 jika tidak ada" />
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <select {...register('province')} className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
                        ...
                        <div>
                            <label className="block text-sm font-medium text-gray-700 mb-1.5">Kota <span className="text-red-500">*</span></label>
                            <select {...register('city')} className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
                    ...
                    <Input {...register('address')} label="Alamat Lengkap" error={errors.address?.message} required />
                    <Input {...register('location_maps')} label="URL Google Maps (opsional)" placeholder="https://maps.google.com/..." error={errors.location_maps?.message} />
                    <Input {...register('facilities')} label="Fasilitas (pisah dengan koma)" placeholder="Parkir, Loker, Shower" error={errors.facilities?.message} required />
                ...
                <div className="flex gap-3 px-6 py-4 border-t border-gray-200 flex-shrink-0">
                    ...
                    <Button fullWidth isLoading={isPending} disabled={isPending} onClick={handleSubmit(onSubmit)}>
