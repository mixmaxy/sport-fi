...
export const CategoryFormModal = ({
  ...
  const onSubmit = (data: CategoryFormData) => {
    const resolvedImageUrl = imageUrl || category?.imageUrl || "";
    if (!isEditing && !resolvedImageUrl) {
      setUploadError("Gambar kategori wajib diunggah untuk kategori baru.");
      return;
    }
    setServerError(null);
    ...
    if (isEditing && category) {
      updateCategory(
        {
          id: category.id,
          name: data.name,
          imageUrl: resolvedImageUrl || category.imageUrl,
        },
        {
          onSuccess: () => {
            onSaved?.();
      ...
      createCategory(
        { name: data.name, imageUrl: resolvedImageUrl },
        {
        onSuccess: () => {
          onSaved?.();
          onClose();
